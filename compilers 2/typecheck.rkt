...
(define (init-valEnv)
  (let ([venv (empty-env)])
    (extend-env venv 'print (types:FunValue 'print (list (types:NameTypePair (types:make-VarValue (types:make-StringType)) 's)) (types:make-VoidType) ))
    (extend-env venv 'printi (types:FunValue 'printi (list (types:NameTypePair (types:make-VarValue (types:make-IntType)) 'x)) (types:make-VoidType)))
    (extend-env venv 'flush (types:FunValue 'flush '() (types:make-VoidType)))
    (extend-env venv 'getChar (types:FunValue 'getChar '() (types:make-StringType)))
    (extend-env venv 'ord (types:FunValue 'ord (list (types:NameTypePair (types:make-VarValue (types:make-StringType)) 's)) (types:make-IntType)))
    (extend-env venv 'chr (types:FunValue 'chr (list (types:NameTypePair (types:make-VarValue (types:make-IntType)) 'i)) (types:make-StringType)))
    (extend-env venv 'size (types:FunValue 'size (list (types:NameTypePair (types:make-VarValue (types:make-StringType)) 's)) (types:make-IntType)))
    (extend-env venv 'substring (types:FunValue 'substring (list (types:NameTypePair (types:make-VarValue (types:make-StringType)) 's)
                                                      (types:NameTypePair (types:make-VarValue (types:make-IntType)) 'first)
                                                      (types:NameTypePair (types:make-VarValue (types:make-IntType)) 'n)) (types:make-StringType)))
    (extend-env venv 'concat (types:FunValue 'concat (list (types:NameTypePair (types:make-VarValue (types:make-StringType)) 's1)
                                                   (types:NameTypePair (types:make-VarValue (types:make-StringType)) 's2)) (types:make-StringType)))
    (extend-env venv 'not (types:FunValue 'not (list (types:NameTypePair (types:make-VarValue (types:make-BoolType)) 'i)) (types:make-BoolType)))
    (extend-env venv 'Exit (types:FunValue 'Exit (list (types:NameTypePair (types:make-VarValue (types:make-IntType)) 'i)) (types:make-VoidType)))
...
(define (typecheck ast tenv venv loop?)
  (let ([type-of-expr 
                              ...
                              (cond
                                ...
                                [(eq? type #f) (if (types:PengType? t1)
                                                   ...
                                                   (let ([varval (types:make-VarValue t1)])
                                                     ...
                                                     (add-note ast 'var varval) ;changed-tz
                                                     varval))]
    ...
    [(NameType name kind next) (typecheck-type-decls ast tenv)]
    ...
    [(RecordType name fields next) (typecheck-type-decls ast tenv)]

    ; handle array declarations
    [(ArrayType name fields next) (typecheck-type-decls ast tenv)]

    ; now for function declarations!
    [(FunDecl name args rettype body next) (typecheck-fun-decls ast tenv venv loop?)]
                      ...
                      (cond
                        ...
                        [(types:VarValue? t1)
                         (let ([ty (types:actual-type (types:VarValue-type t1))])
                           ...
                           ; we need to add this so our later phase can access the
                           ; which var expression is associated with which declaration
                           (add-note ast 'var t1)
                           ty)]
                        [else
                         (begin
    ...
    [(MathExpr e1 op e2) (let ([t1 (types:actual-type (typecheck e1 tenv venv loop?))]
                           ...
                           (cond
                             ...
                             [else (let ([retty (types:make-IntType)])
                                     ;(add-note ast 'type retty)
                                     retty)]))]

    ; bool expressions are more complicated because it depends on the operator
    [(BoolExpr e1 op e2) (let ([retty (typecheck-boolexprs ast e1 op e2 tenv venv loop?)])
                           ;(add-note ast 'type retty)
                           retty)]
    ...
    [(LogicExpr e1 op e2) (let ([t1 (types:actual-type (typecheck e1 tenv venv loop?))]
                           ...
                           (cond
                             ...
                             [else
                              (let ([retty (types:make-BoolType)])
                                ;(add-note ast 'type retty)
                                retty)]))]
    
    ; handle record creation, here we pass the ast mainly for logging error purposes
    [(NewRecordExpr name assignments) (let ([retty
                                             (typecheck-record-creation ast name assignments tenv venv loop?)])
                                        ;(add-note ast 'type retty)
                                        retty)]
    ...
    [(ArrayExpr name-expr index) (let*
                                   ...
                                   (cond
                                     ...
                                     [else
                                      ; note, here we return the element type since we're
                                      ; accessing via an index into the array
                                      (let ([retty (types:ArrayType-element-type arrty)])
                                        ;(add-note ast 'type retty)
                                        retty)]))]
    ...
    ; only add a note if it's not a void type
    (unless (void? type-of-expr)
    ...
    (cond
      ...
      [else
       (begin
         ...
         ; add the varvalue to the with expression so when we process it, we can grab it to set its result
         (add-note node 'var counterty)
    ...
    (if (= (length args) (length params))
        (begin
          (for-each (λ (arg param)
                      (let ([argty (typecheck arg tenv venv loop?)]
                            [paramty (types:actual-type (types:VarValue-type (types:NameTypePair-type param)))])
                        (cond
        ...
        (begin
          (log-typeerror "~a arguments given to ~a, but it requires ~a parameters"
                         node (length params) name (length args))
          returnty))))
                          ...
                          (let* ([paramname (TypeField-name typefield)]
                            ...
                            (if (eq? argty #f)
                                (begin
                                  ...
                                  (types:NameTypePair (types:make-VarValue (types:make-VoidType)) paramname))
                                ; otherwise, if argty is not false, it's a valid type, so we can create a name/type pair
                                (begin
                                  ;(printf "argty is: ~a \n" argty)
                                  (types:NameTypePair
                                   (types:VarValue argty #f #f #f 0) ;changed-tz
                                   paramname))))) args)])
              (cond
                [(eq? rty #f) (log-typeerror "~a is an unknown return type for your function" fun rettype)]
                ; now extend the environment
                [else (let ([funval (types:FunValue name params rty)]) ;changed-tz
                        (extend-env venv name funval)
                        (add-note fun 'funval funval))]))]
     ...
     (match fun
       [(FunDecl name args rettype body _)     
        (let* ([tenv1 (push-scope tenv)]
               ...
               [varvaluelists (map (λ (param) (types:NameTypePair-type param)) (types:FunValue-parameters funty))])
          (for-each (λ (param)
                      ; now extend the new environment with VarValues of the parameters of the function
                      (extend-env venv1
                                  (types:NameTypePair-name param)
                                  (types:NameTypePair-type param)))
          ...
          ;(types:set-FunValue-label! funty (make-global-result))
...
(define (fieldassigns-correct? node assignments nametypepairs tenv venv loop?)
  (cond
    ...
    [else (let* ([assign (first assignments)]
                 ...
                 [ntpairtype (types:actual-type (types:NameTypePair-type ntpair))])
            ...
            (if (eq? fieldname ntpairname)
                ...
                (cond
                  ...
                  [else (begin (log-typeerror "types do not match for record field (~a) and expression type (~a)" node
                                              ntpairname etype) #f)])
                (begin (log-typeerror "names and order of field assignments must match record declaration" node) #f)))]))

(define (fieldass-correct? node assignments nametypepairs tenv venv loop?)
  (printf "entering new field assign")
  (foldl
   (λ (fieldassign nametypepair last)
     (printf "entering new field assign foldl")
     (let ([etype (begin
                    (printf "typechecking field assign")
                    (typecheck (FieldAssign-expr fieldassign) tenv venv loop?))]
           [fieldname (begin
                        (printf "getting field assign name")
                        (FieldAssign-name fieldassign))]
           [pairname (types:NameTypePair-name nametypepair)]
           [pairtype (types:NameTypePair-type nametypepair)])
       (printf "field assign pairs: ~a~n" nametypepair)
       (if (eq? fieldname pairname)
           (cond
             [(or (types:type=? etype pairtype) (peng-match? etype pairtype))
              (begin
                (printf "types=? or peng-match?")
                (and #t last))]
             [else (begin
                     (log-typeerror "types do not match for record field (~a) and expression type (~a)" node
                                    pairname etype)
                                    #f)])
           (begin
             (log-typeerror "names and order of field assignments must match record declarations" node) #f))))
  #t assignments nametypepairs ))
     ...
     (for-each-type (λ (type)
                      (match type
                        ...
                        [(ArrayType name kind _)
                         (let* ([namety (apply-env tenv name)]
                                [ty (apply-env tenv kind)]
                                [array-type (types:make-ArrayType name ty)])
                           (printf "adding to node ~a, type ~a~n" type array-type)
                           (add-note type 'type array-type)
                           (types:bind-nametype! namety array-type))]
                        ...
                        [(RecordType name fields _)
                         (let* ([recordty (apply-env tenv name)]
                               ; use map to create a list from the typefields to nametypepairs
                                ...
                                [record-type (types:make-RecordType name recfields)])
                           ; add a note for this type
                           (add-note type 'type record-type)
                           (types:bind-nametype! recordty record-type))]))
                    decl)
